...
这个分组常见于 `search.find` 和 `search.search`。

| 字段 | 含义 |
| --- | --- |
| `summary.search.vlm.duration_ms` | 查询理解或意图分析阶段的耗时 |
| `summary.search.embedding.duration_ms` | 查询 embedding 生成耗时 |
| `summary.search.vector_db.duration_ms` | 向量库检索累计耗时，可能包含多次查询 |
| `summary.search.rerank.duration_ms` | rerank 累计耗时，可能包含多轮 rerank |

说明：

- `search.find` 一般不会产出 `summary.search.vlm.duration_ms`
- 没有触发 rerank 时，不会返回 `summary.search.rerank.duration_ms`
- 未执行或结果为 `0` 的字段会被省略
