...
class TestSafeExtractZipNormal:
    ...
    def test_handles_special_characters_in_filenames(self, tmp_path: Path) -> None:
        ...
        assert (dest / "bad@file?.txt").exists()
...
class TestSafeExtractZipSymlink:
    ...
    def test_symlink_entry_is_skipped_without_type_error(self, tmp_path: Path) -> None:
        """Symlink members should be skipped based on mode bits, not path APIs."""
        dest = tmp_path / "out"
        dest.mkdir()
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            info = zipfile.ZipInfo("link name")
            info.external_attr = (stat.S_IFLNK | 0o777) << 16
        ...
        assert not (dest / "link name").exists()
        assert (dest / "safe file.txt").read_text() == "ok"
